...
func (sf *Frame) PopulateShaders() {
	sf.Shaders = make([]Shader, len(sf.Frame.Meshes))
	for i, m := range sf.Frame.Meshes {
...
func (f *Frame) intersect(ray line.Line) *Intersection {
	out := &Intersection{
		Ray:   ray,
		Frame: f.Frame,
	}
	for _, mi := range f.Intersectors {
...
func (sf *Frame) trace(ray line.Line, depth int) *material.Color {
	ctx := &Context{
