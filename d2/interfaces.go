...
func (AssertV1) AssertEqual(actual interface{}) error {
	pv, ok := actual.(Pt1V1)
	if !ok {
		return geomtest.TypeMismatch(Pt1V1(nil), actual)
	}

	fn := make([]func(float64) V, 2, 3)
	fn[0] = func(t0 float64) V {
		return pv.Pt1(t0 + geomtest.Small).Subtract(pv.Pt1(t0)).Multiply(geomtest.Big)
	}
	fn[1] = pv.V1
