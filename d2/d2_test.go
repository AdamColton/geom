...
func TestAssertEqualFailures(t *testing.T) {
	err := Pt{1, 2}.AssertEqual(V{1, 2})
	assert.Equal(t, geomtest.TypeMismatchCode, err.(geomtest.ErrWrapper).Code)

	err = Pt{1, 2}.AssertEqual(Pt{2, 1})
	assert.Equal(t, geomtest.NotEqualCode, err.(geomtest.ErrWrapper).Code)

	err = V{1, 2}.AssertEqual(Pt{1, 2})
	assert.Equal(t, geomtest.TypeMismatchCode, err.(geomtest.ErrWrapper).Code)

	err = V{1, 2}.AssertEqual(V{2, 1})
	assert.Equal(t, geomtest.NotEqualCode, err.(geomtest.ErrWrapper).Code)
}
